...
const (
	...
	queryGetMonitorTambak              = `SELECT monitor_tambak_id, ph, do, suhu, waktu_tanggal, keterangan FROM monitor_tambak WHERE tambak_id = ? AND DATE(waktu_tanggal) = ? ORDER BY waktu_tanggal ASC`
	QueryUpdateTambak                  = `
		UPDATE tambak
		SET 
			nama_tambak = ?,
