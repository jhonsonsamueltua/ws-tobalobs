...
const (
	...
	queryGetAllTambakForAdmin = "SELECT tambak_id, nama_tambak, panjang, lebar, jenis_budidaya, tanggal_mulai_budidaya, usia_lobster, jumlah_lobster, jumlah_lobster_jantan, jumlah_lobster_betina, status FROM tambak WHERE status = 'aktif' ORDER BY tambak_id DESC"
	...
	queryGetLastMonitorTambak          = "SELECT t.tambak_id, t.nama_tambak, m.ph, m.do, m.suhu, IFNULL(m.waktu_tanggal,'') as waktu_tanggal, IFNULL(m.keterangan,'') as keterangan, t.panjang, t.lebar, t.jumlah_lobster FROM tambak as t LEFT JOIN monitor_tambak as m ON t.tambak_id = m.tambak_id WHERE t.tambak_id = ? ORDER BY m.monitor_tambak_id DESC LIMIT 1"
	...
	queryUpdateGuideline = `
		...
		WHERE guideline_id = ?
	`
	queryGetTunnel    = "SELECT id, ip, port FROM tunnel WHERE id = ?"
	queryUpdateTunnel = `
		UPDATE tunnel
		SET 
			ip = ?,
